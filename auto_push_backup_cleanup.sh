#!/bin/bash
# auto_push_backup_cleanup.sh - smart multi-type auto push with backups, log rotation, notifications, and automatic backup cleanup
...
while true; do
    ...
    # Cleanup old backups
    find "$BACKUP_DIR" -type f -name "FairFlight_PWA_backup_*.tar.gz" -mtime +$MAX_BACKUP_DAYS -exec rm -f {} \;
